new Vue({
    ...
    data: {
      comments: [],
      liked_comments: [], // Make sure 'liked_comments' is defined in the data
      newCommentText: "",
    ...
    methods: {
        ...
        if (this.newCommentText.trim() !== "") {
          if (this.replyingToComment) {
            ...
            try {
              ...
              var _data = {
                is_like:  false,
                p_post_id: parseInt(getDecryptQueryStringParameter("cmt").split("=")[1]),
                is_dislike: false,
                is_view:false,
                is_comment:true
               };
      ...
      my_liked_comments() {
        const likeCommentUrl = supabase_url() + "/get_likes_comments_id_by_post_id";
        const headers = {
          Authorization: "Bearer " + supabase_KEY(),
          apikey: supabase_KEY(),
          "Content-Type": "application/json",
        ...
        try {
          axios.post(likeCommentUrl, _my_liked_comments_d, { headers })
          .then((suc)=>{
      ...
      likeComment(commentSeq) {
        ...
        const _like_data = {
          ...
          p_user_id: parseInt(getSessionData("USER_ID")),
          p_resource_type: 'COMMENT',
          p_post_seq: parseInt(getDecryptQueryStringParameter("cmt").split("=")[1]),
        };
  
        axios.post(
          likeCommentUrl, _like_data, { headers }
        ).then((res) => {
          if (res.status == 200) {
      ...
      updateLikesCount(likeAction, commentSeq) {
        ...
        if (liElement) {
            ...
            const newLikesCount = currentLikesCountNumber + (likeAction === 'inserted' ? 1 : -1);
      ...
      isButtonActive(commentSeq) {
        const isAlreadyLiked = this.liked_comments && this.liked_comments.length > 0 &&
         this.liked_comments.includes(commentSeq);
         return isAlreadyLiked;
      },
      showChildComments(commentSeq){
         const liChildCommentId=`liChildComments-${commentSeq}`;
         const liChildCommentElements=document.getElementsByClassName(liChildCommentId);
